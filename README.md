...
Use `build.entrypoint` to point at the binary generated by `build.cmd` and keep every argument in `build.args_bin` (or on the command line). Entrypoint paths are resolved relative to `root` whenever the value contains a path separator, but bare command names are resolved via `$PATH`, which makes it easy to run helpers such as `dlv` without copying their binaries into your project. The legacy `build.bin` field is still honored for this minor release, but it will be removed in the future, so prefer the entrypoint form going forward.
...
entrypoint = "dlv"
args_bin = [
  "exec", "--accept-multiclient", "--log", "--headless", "--continue",
