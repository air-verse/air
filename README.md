...
Use `build.entrypoint` to point at the binary generated by `build.cmd` and keep every argument in `build.args_bin` (or on the command line). Entrypoint paths are resolved relative to `root`, which lets Air locate executables inside directories that contain spaces without having to encode arguments inside `build.bin`. The legacy `build.bin` field is still honored for this minor release, but it will be removed in the future, so prefer the entrypoint form going forward.
