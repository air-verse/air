...
func Test_isPowershell(t *testing.T) {
	tests := []struct {
		name string
		want bool
	}{
		...
		{
			name: "Is running on CMD?",
			want: false,
		},
		{
			name: "Is running on PowerShell?",
			want: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := isPowershell(); got != tt.want {
				t.Errorf("isPowershell() = %v, want %v", got, tt.want)
			}
		})
	}
}
